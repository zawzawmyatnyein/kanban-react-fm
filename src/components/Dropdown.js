...
function Dropdown() {
  ...
  const [isEBMOpen, setIsEBMOpen] = useState(false)

  const openCMModal = () => setIsCMOpen(true)
  const closeCmModal = () => setIsCMOpen(false)
  const openEBModal = () => setIsEBMOpen(true)
  const closeEBModal = () => setIsEBMOpen(false)
  ...
  return (
    <>
        ...
        <Menu as="div" className="relative inline-block text-left">
          <div>
            <Menu.Button className="inline-flex w-full justify-center rounded-lg py-1 text-sm font-medium text-white hover:bg-light-gray dark:hover:bg-black/30 focus:outline-none focus:bg-light-gray dark:focus:bg-black/30 focus:ring-1 focus:ring-medium-gray/20">
              <EllipsisVerticalIcon className="h-7 w-7 text-medium-gray" aria-hidden="true" />
            </Menu.Button>
          </div>
          <Transition
            as={Fragment}
      ...
      <EditBoardModal isOpen={isEBMOpen} closeModal={closeEBModal} />
      <ConfirmationModal isOpen={isCMOpen} closeModal={closeCmModal} />
    </>
  )
}
